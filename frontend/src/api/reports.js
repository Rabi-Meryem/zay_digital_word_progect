...
export const downloadClientReport = async (exportFormat, filters = {}) => {
  const response = await apiClient.get('/reports/generate-client/', {
    params: { export_format: exportFormat, ...filters },
    responseType: 'blob',
  })
  const ext = exportFormat === 'excel' ? 'xlsx' : exportFormat
  ...
  const link = document.createElement('a')
  link.href = url
  link.setAttribute('download', `mon_rapport.${ext}`)
  document.body.appendChild(link)
  link.click()
  link.remove()
}
